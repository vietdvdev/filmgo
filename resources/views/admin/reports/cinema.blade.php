...
@php
    ...
    $hasFilter = $cinemaId || $day || $month || $year || $from || $to;
    ...
    <div class="mt-6 md:mt-10 mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-1">
            ...
            <span class="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-500 bg-white border border-zinc-200 rounded-xl px-3 py-1.5">
                ...
                @if($cinemaId)
                    &nbsp;·&nbsp;
                    <span class="text-red-600">{{ $allCinemas->firstWhere('id', $cinemaId)?->name }}</span>
                @endif
            </span>
        </div>
    ...
    <form method="GET" action="{{ route('admin.reports.cinema') }}"
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <div class="lg:col-span-2">
                <label class="block text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1">Rạp chiếu</label>
                <select name="cinema_id"
                    ...
                    @foreach($allCinemas as $c)
                        <option value="{{ $c->id }}" @selected($cinemaId == $c->id)>{{ $c->name }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-gradient-to-br from-red-600 to-red-700 rounded-2xl p-5 shadow-md shadow-red-200 flex flex-col gap-3">
            ...
            <p class="text-[11px] text-red-200">
                @if($cinemaId)
                    {{ $allCinemas->firstWhere('id', $cinemaId)?->name }}
                @else
                    tất cả {{ $allCinemas->count() }} rạp
                @endif
            </p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-zinc-50 border-b border-zinc-100 text-[10px] font-black uppercase tracking-widest text-zinc-400">
                        ...
                        <th class="px-6 py-3.5 text-left">Rạp chiếu</th>
                ...
                <tbody class="divide-y divide-zinc-100">
                    ...
                    <tr class="hover:bg-zinc-50/60 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-right">
                            @if($cinema['ticket_count'] > 0)
                                <span class="inline-flex items-center gap-1 bg-blue-50 text-blue-700 font-bold text-xs px-2.5 py-1 rounded-lg">
                                    ...
                                    {{ number_format($cinema['ticket_count']) }}
                                </span>
                            @else
                        ...
                        <td class="px-6 py-4 text-right">
                            <span class="font-black text-red-600">{{ fmtVnd($cinema['total_revenue']) }}</span>
                        </td>
                        <td class="px-6 py-4 text-right">

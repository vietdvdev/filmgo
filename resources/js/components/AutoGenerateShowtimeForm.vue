...
// Props (If passing list of movies or API urls from Laravel layout)
...
// Lifecycle hooks
onMounted(() => {
  // Preload movies if provided by props, otherwise we could fetch
  if (props.moviesData && props.moviesData.length > 0) {
    moviesList.value = props.moviesData;
  } else if (window.__SHOWTIME_DATA__ && window.__SHOWTIME_DATA__.movies) {
    moviesList.value = window.__SHOWTIME_DATA__.movies;
  }

  // Fetch rooms. AutoGenerate usually operates on the manager's assigned cinema
...
const fetchAssignedCinemaRooms = async () => {
  try {
    ...
    if (cinemaRes.data && cinemaRes.data.length > 0) {
      // Dùng rạp được phân công đầu tiên để tự động tải các phòng chiếu
...
// Set standard price input quickly
...
const submitAutoGenerate = async () => {
  ...
  try {
    ...
    if (res.data && res.data.success) {
      ...
      // Reset form payload to defaults
      payload.movie_id = '';
      payload.room_id = '';
